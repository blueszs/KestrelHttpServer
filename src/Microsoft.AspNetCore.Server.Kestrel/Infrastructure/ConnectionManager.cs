...
namespace Microsoft.AspNetCore.Server.Kestrel.Infrastructure
{
    public class ConnectionManager
    {
        ...
        public async Task<bool> CloseAllConnections(TimeSpan timeout)
        {
            if (_managerClosed)
            {
            ...
            var allCompletedTask = Task.WhenAll(stopTasks);
            var completedTask = await Task.WhenAny(allCompletedTask, Task.Delay(timeout));

            return completedTask == allCompletedTask;
        }
    }
}
